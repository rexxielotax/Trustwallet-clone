...
export function CTABanner() {
  return (
    ...
    >
      <div className="rounded-[32px] bg-gradient-hero p-8 md:p-16 grid md:grid-cols-2 gap-8 items-center text-white overflow-hidden relative">
        <div>
        ...
        <motion.div
          className="aspect-[4/3] rounded-2xl bg-white/10 grid place-items-center overflow-hidden"
          animate={{ y: [0, -10, 0] }}
          transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
        >
          <img src={developerImg} alt="Developer building on Trust" loading="lazy" className="w-full h-full object-contain" />
        </motion.div>
      </div>
    </motion.section>
  );
